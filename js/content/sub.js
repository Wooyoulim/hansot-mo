...
const store = () => {
  ...
  function displayInfowindow(marker, title) {
    var content =
      '<div style="padding:1.3889vw;z-index:1;">' + title + "</div>";
...
const fqa = () => {
  ...
  let sortArr1 = faqPosts.filter(
    (element) => element.sort === $sortLi[1].textContent
  );
  let sortArr2 = faqPosts.filter(
    (element) => element.sort === $sortLi[2].textContent
  );
  let sortArr3 = faqPosts.filter(
    (element) => element.sort === $sortLi[3].textContent
  );
  let sortArr4 = faqPosts.filter(
    (element) => element.sort === $sortLi[4].textContent
  );
  let sortArr5 = faqPosts.filter(
    (element) => element.sort === $sortLi[5].textContent
  );
